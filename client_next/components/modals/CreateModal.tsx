...
export default function CreateModal({onClose,onPostCreated}: CreateModalProps) {
    ...
    return (
        <>
            ...
            <div className={`fixed z-60 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90vw] md:w-[40vw] bg-white border dark:border-white/20 rounded-lg shadow-xl p-6 transition-all duration-200 ${visible ? "opacity-100 scale-100" : "opacity-0 scale-95"}`}>
                ...
                <div className="mt-5 mb-3 flex items-center gap-3">
                    ...
                    <select value={intent} onChange={(e) => setIntent(e.target.value)} className="border border-black/20 cursor-pointer px-5 rounded-md py-1 bg-white outline-none">
                ...
                <textarea placeholder="What's on your mind?" value={content} onChange={(e) => setContent(e.target.value)} className="w-full h-32 resize-none border border-black/10 rounded-lg p-3 outline-none" />
